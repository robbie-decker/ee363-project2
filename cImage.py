...
if __name__ == '__main__':
    ...
    for row in range(newImage.getHeight()):
        for col in range(newImage.getWidth()):
            ...
            if row == 0 or row == height-3 or col == 0 or col == width-1:
                v.red = 255
                v.green = 0
                v.blue = 0
            else:
    ...
    # borderImage = ImageOps.expand(newImage, 10,  100)
    
    newImage.setPosition(newImage.getWidth()+1,10)
    newImage.draw(win)

    # borderImage.setPosition(newImage.getWidth()+1,10)
    # borderImage.draw(win)
